  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-6 border-b border-slate-100 dark:border-zinc-800 pb-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-muted text-sm mt-1">Deep dive into campaign spending and patterns.</p>
          </div>
          <div className="flex items-center gap-3">
            <button 
              onClick={() => setCompareMode(!compareMode)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm transition-all border ${compareMode ? 'bg-indigo-50 border-indigo-200 text-indigo-700 dark:bg-indigo-900/30 dark:border-indigo-800 dark:text-indigo-400' : 'bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 text-slate-600 dark:text-muted'}`}
            >
              <Calendar size={16} /> {compareMode ? 'Disable Comparison' : 'Compare Periods'}
            </button>
            <button 
              onClick={exportToCSV}
              className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm transition-all shadow-md hover:shadow-lg active:scale-[0.98] active:nm-inset"
            >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <input 
                      ...
                      className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm text-slate-700 dark:text-muted w-full"
                    />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { label: 'Total Spent', value: `$${stats.totalSpent.toLocaleString()}`, color: 'text-slate-900 dark:text-main', icon: DollarSign, trend: (stats as any).spentDiff },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h4 className="font-bold text-slate-800 dark:text-main text-lg">Spending Trend</h4>
            {compareMode && (
        ...
        <div className="p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm">
          <h4 className="font-bold text-slate-800 dark:text-main mb-6 text-lg">Platform Distribution</h4>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm lg:col-span-2">
          <h4 className="font-bold text-slate-800 dark:text-main mb-6 text-lg">Ad Goals Allocation</h4>
        ...
        {role === 'Admin' && (
          <div className="p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm lg:col-span-2">
            <h4 className="font-bold text-slate-800 dark:text-main mb-6 text-lg">Top Spending Users</h4>
                ...
                <div key={user.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      <p className="text-sm font-bold text-slate-700 dark:text-muted">{user.name}</p>
