...
const Navbar: React.FC<NavbarProps> = ({ onLoadMoney, onSettings }) => {
  ...
  const isAdminPage = pathname === '/admin';
    ...
    <header className="sticky top-4 z-50 mx-4 md:mx-8 mb-6 nm-flat px-4 md:px-8 py-3.5 rounded-2xl transition-all duration-300">
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="absolute top-[calc(100%+1rem)] left-0 right-0 glass rounded-2xl p-4 flex flex-col gap-3 md:hidden shadow-2xl animate-in slide-in-from-top-4 fade-in duration-200 z-50 border border-white/5">
          {profile && (
