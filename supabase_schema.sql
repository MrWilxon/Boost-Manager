...
CREATE TABLE IF NOT EXISTS public.notifications (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE, -- if null, it's a global admin alert
    title TEXT NOT NULL,
    message TEXT NOT NULL,
    type TEXT NOT NULL, -- e.g., 'boost_created', 'balance_requested', 'boost_approved'
...
-- Trigger on new Boost Request
CREATE OR REPLACE FUNCTION public.notify_admin_new_boost()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER on_new_boost_request
    AFTER INSERT ON public.boost_requests
    FOR EACH ROW EXECUTE FUNCTION public.notify_admin_new_boost();

-- Trigger on Boost Approval/Rejection
CREATE OR REPLACE FUNCTION public.notify_user_boost_status()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER on_boost_status_change
    AFTER UPDATE ON public.boost_requests
    FOR EACH ROW EXECUTE FUNCTION public.notify_user_boost_status();

-- Trigger on new Balance Request
CREATE OR REPLACE FUNCTION public.notify_admin_new_balance()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO public.notifications (user_id, title, message, type)
...
CREATE OR REPLACE TRIGGER on_new_balance_request
    AFTER INSERT ON public.balance_requests
    FOR EACH ROW EXECUTE FUNCTION public.notify_admin_new_balance();

-- Trigger on Balance Request Approval/Rejection
CREATE OR REPLACE FUNCTION public.notify_user_balance_status()
RETURNS TRIGGER AS $$
BEGIN
    IF OLD.status = 'Pending' AND NEW.status != 'Pending' THEN
        INSERT INTO public.notifications (user_id, title, message, type)
...
CREATE OR REPLACE TRIGGER on_balance_status_change
    AFTER UPDATE ON public.balance_requests
    FOR EACH ROW EXECUTE FUNCTION public.notify_user_balance_status();
